...
from dt.data.preprocess.validators import (check_range, check_rate_of_change,
                                           check_stuck)
...
class ValidationProcessor(BaseProcessor):
    ...
    def process(self, context: ProcessingContext) -> ProcessingContext:
        ...
        # Range check
        range_passed, range_flag = check_range(reading, sensor_config.range)
        if not range_passed:
            logger.info(
                f"Range validation failed: sensor_id={sensor_id} "
                f"value={reading.value} range=[{sensor_config.range.min}, {sensor_config.range.max}]"
            )
        ...
        roc_passed, roc_flag = check_rate_of_change(reading, previous_valid, sensor_config.roc)
        if not roc_passed:
            logger.info(
        ...
        stuck_passed, stuck_flag = check_stuck(history, sensor_config.stuck)
        if not stuck_passed:
