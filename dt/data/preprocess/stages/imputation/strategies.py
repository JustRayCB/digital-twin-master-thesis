...
def build_imputation_strategy(params: ImputationConfig | None) -> ImputationStrategy:
    """Instantiate the imputation strategy configured for a sensor.

    Parameters
    ----------
    params : ImputationConfig or None
        Configuration object specifying the desired imputation strategy and
        its parameters. When ``None``, defaults to forward-fill strategy.
    ...
    """
    if params is None:
        params = ForwardFillImputationConfig()

    if isinstance(params, ForwardFillImputationConfig):
        return ForwardFillWithDecay(params)
    if isinstance(params, WindowAverageImputationConfig):
        return WindowAverageImputation(params)
    if isinstance(params, LinearExtrapolationImputationConfig):
        return LinearExtrapolationImputation(params)

    raise ValueError(f"Unsupported imputation strategy parameters: {params}")
