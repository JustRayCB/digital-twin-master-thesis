...
from dt.data.preprocess.core.state import FlatlineRecord, StateProvider
from dt.data.preprocess.stages.imputation import (
...
def test_build_strategy_rejects_unknown_strategy_name() -> None:
    ...
    with pytest.raises(ValueError):
        build_imputation_strategy(_UnknownImputationConfig())  # type: ignore[arg-type]
...
def test_build_strategy_uses_default_when_missing_config() -> None:
    ...
    strategy = build_imputation_strategy(None)
...
def test_build_strategy_selects_window_average_strategy() -> None:
    ...
    strategy = build_imputation_strategy(
        WindowAverageImputationConfig(window_seconds=30, min_samples=2, max_gap_seconds=120)
    )
...
def test_build_strategy_selects_linear_interpolation() -> None:
    ...
    strategy = build_imputation_strategy(
        LinearExtrapolationImputationConfig(window_seconds=180, max_gap_seconds=300)
    )
